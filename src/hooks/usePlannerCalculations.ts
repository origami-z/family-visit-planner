...
import { differenceInDays, parseISO, isWithinInterval, isBefore, isAfter, addDays, subYears, format } from 'date-fns';
import { Trip, FamilyMember, EmptyPeriod, MemberStats } from '@/types/planner';

export function useStayDuration(trips: Trip[], memberId: string, referenceDate: Date = new Date()) {
  return useMemo(() => {
...
export function useMemberStats(members: FamilyMember[], trips: Trip[], yearLimit: number): MemberStats[] {
  return useMemo(() => {
    const today = new Date();
    
    return members.map(member => {
      const memberTrips = trips.filter(t => t.memberId === member.id).sort((a, b) => 
      ...
      memberTrips.forEach(trip => {
        ...
        const overlapStart = isBefore(start, oneYearAgo) ? oneYearAgo : start;
        const overlapEnd = isAfter(end, today) ? today : end;
      ...
      member.warnings.filter(w => w.enabled).forEach(warning => {
