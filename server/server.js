...
async function getStartManifest() {
  const { tsrStartManifest } = await import("./assets/_tanstack-start-manifest_v-ClmwNksq.js");
...
const handleServerAction = async ({
  ...
  if (regex === void 0) {
    regex = new RegExp(`${"/family-visit-planner/_serverFn/"}([^/?#]+)`);
  }
...
function createStartHandler(cb) {
  const ROUTER_BASEPATH = "family-visit-planner";
  let startRoutesManifest = null;
  let startEntry = null;
  let routerEntry = null;
  const getEntries = async () => {
    if (routerEntry === null) {
      routerEntry = await import("./assets/router-d_sCbnpZ.js");
    }
    if (startEntry === null) {
      ...
      const requestHandlerMiddleware = handlerToMiddleware(
        async ({ context }) => {
          const response2 = await runWithStartContext(
            ...
            async () => {
              try {
                if (href.startsWith("/family-visit-planner/_serverFn/")) {
